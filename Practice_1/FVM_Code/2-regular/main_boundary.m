...
for jj=1:M
    ...
    x(1) = (x_cp(2)+x_cp(1))/2.0;
    x(N+1) = (x_cp(N+2)+x_cp(N+1))/2.0;
...
figure
plot(log(ll),-log(norml2),'r', log(ll), -log(normh1),'blue', log(ll),log(ll)+1.5,'green');
title('Error');
legend('L^2 Norm', 'H^1 norm', 'x')