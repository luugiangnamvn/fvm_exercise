...
for jj=1:M
    ...
    for i_iter=1:N
        if(i_iter==1)
            ...
            if(i_iter==N)
                for i=1:i_iter-2
                    a1=-1/((x(N+1)-x(N))*(x_cp(N+1)-x_cp(N)));
                    A(i_iter,i)=a1/(x(N+1)-x(N))*(x(i+1)-x(i));
                end
                a1=-1/((x(N+1)-x(N))*(x_cp(N+1)-x_cp(N)));
                A(i_iter,i_iter-1)=a1+a1/(x(N+1)-x(N))*(x(i_iter)-x(i_iter-1));
                A(i_iter,i_iter)=0;
            else
...
figure
plot(log(ll),-log(norml2),'r', log(ll), -log(normh1),'blue', log(ll),log(ll)+2, 'black', log(ll),2*log(ll)+1.5,'green');
title('Error');
legend('L^2 Norm', 'H^1 norm', 'x', '2x')