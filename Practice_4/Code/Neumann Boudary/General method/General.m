...
norml2_x=zeros(M,1); % norm l2 at T;
normh1_x=zeros(M,1); % norrm h1 at T;
phi =1/5;            % Choose an arbitrary  number
...
for jj=1:M
    ...
    %% Create for computing
    A = zeros(N,N);  % Matrix A
    u = zeros(N,T);  % Matrix value of u
    u0 = zeros(N,1); % u(x,0)
    psi_a = zeros(T);  %psi_(a,t)
    psi_b = zeros(T);  %psi_(b,t)
    
    %% Input iterior
    for i_iterx=1:N
        u0(i_iterx) = u_ex(x_cp(i_iterx+1),0); % u_0(x,0)
    end
    
    for i_itert = 1:T
        psi_a(i_itert) = ps(a,t(i_itert));  %psi_a(t)
    end
    
    for i_itert = 1:T
        psi_b(i_itert) = ps(b,t(i_itert));  % psi_b(t)
    end
    ...
    for i_itert =1:T
        for i_iterx =1:N
            if(i_iterx ==1)
                ...
                F(i_iterx,i_itert) = (fun(x(i_iterx+1),t(i_itert))+fun(x(i_iterx),t(i_itert)))/2 - alpha*(x_cp(i_iterx+1)-x_cp(i_iterx))*psi_a(i_itert); 
            elseif (i_iterx ==N)
                gamma = kappa/((x(i_iterx+1)-x(i_iterx))*(x_cp(i_iterx+2)-x_cp(i_iterx+1)));
                F(i_iterx,i_itert) = (fun(x(i_iterx+1),t(i_itert))+fun(x(i_iterx),t(i_itert)))/2 + gamma*(x_cp(i_iterx+2)-x_cp(i_iterx+1))*psi_b(i_itert); 
            else
    ...
    for i_iterx = 1:N
        ...
        gamma = kappa/((x(i_iterx+1)-x(i_iterx))*(x_cp(i_iterx+2)-x_cp(i_iterx+1)));
        if(i_iterx ==1)
            A(i_iterx,i_iterx) = - gamma;
            A(i_iterx,i_iterx+1) = gamma;
        elseif(i_iterx == N)
            A(i_iterx,i_iterx) = -alpha;
    ...
    u(:,1) = u0(:,1);
    for i_itert = 2:T
        u(:,i_itert) = (eye(N,N)-phi*k*A)^(-1)*((eye(N,N)+(1-phi)*k*A)*u(:,i_itert-1) + k*(phi*F(:,i_itert)+(1-phi)*F(:,i_itert-1)));        
    end        
    ...
    for i_itert=1:T
        for i_iterx = 1:N+2
            if(i_iterx == 1)
                u_dis(i_iterx,i_itert) = u(i_iterx,i_itert) - (x_cp(i_iterx+1)-x_cp(i_iterx))*psi_a(i_itert);
            elseif(i_iterx == N+2)
                u_dis(i_iterx,i_itert) = u(i_iterx-2,i_itert) + (x_cp(i_iterx)-x_cp(i_iterx-1))*psi_b(i_itert);
            else
...
figure
plot(log(ll.^(1/2)),-log(normh1),'b',log(ll.^(1/2)),log(ll.^(1/2)),'c');
