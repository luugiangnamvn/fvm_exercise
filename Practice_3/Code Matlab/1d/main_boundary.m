...
%   bang Phuong phap the tich huu han voi bien Dirichlet thuan nhat va 
%                   gioi thieu cac xap xi cho f 
%%                                                                       %%
clear all
close all
clc
...
%                 end
                if(i==1)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2)- c0*u_exact(x_cp(i+1),0) - a0*u_exact(0,y_cp(j+1));
                elseif(i==N)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - c0*u_exact(x_cp(i+1),0) - b0*u_exact(1,y_cp(j+1));
                else
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - c0*u_exact(x_cp(i+1),0);
                end
        ...
        elseif(j==M)
            for i =1:N
                ...
                b0 = -1/((x(i+1) - x(i))*(x_cp(i+2) - x_cp(i+1)));
                if(i==1)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - d0*u_exact(x_cp(i+1),1) - a0*u_exact(0,y_cp(j+1));
                elseif(i==N)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - d0*u_exact(x_cp(i+1),1) - b0*u_exact(1,y_cp(j+1));
                else
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - d0*u_exact(x_cp(i+1),1) ;
                end
            end
        else
            for i =1:N
                ...
                b0 = -1/((x(i+1) - x(i))*(x_cp(i+2) - x_cp(i+1)));
                if(i==1)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - a0*u_exact(0,y_cp(j+1));
                elseif(i==N)
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2) - b0*u_exact(1,y_cp(j+1));
                else
                    F((j-1)*N+i)=f((x(i)+x(i + 1))/2,(y(j)+y(j+1))/2);
                end
            end
        end
    end
