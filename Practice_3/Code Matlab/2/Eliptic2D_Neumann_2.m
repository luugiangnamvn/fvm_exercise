...
%   bang Phuong phap the tich huu han voi bien Neumann thuan nhat
%%                                                                       %%
clear all
close all
clc
...
for jj=1:iteration
    ...
    C=sparse(N,N);
    D=sparse(N,N); 
    dx = dx/2; %Su dung cho control point tai bien
    dy = dy/2; %Su dung cho control point tai bien
    ...
    for j=1:M
        ...
        for i=1:N
            ...
            if(j == 1)
                if(i==1)
                    B(i,i)=1/(2*dx^2); %Tai bien control point se khac
                    B(i,i+1)=-1/(4*dx^2); %Tai bien control point se khac
                else
                    if(i==N)
                        B(i,i)=1/(2*dx^2); %Tai bien control point se khac
                        B(i,i-1)=-1/(4*dx^2); %Tai bien control point se khac
                    else 
                        B(i,i)=3/(4*dx^2); %Tai bien control point se khac
                        B(i,i-1)=a1;
                        B(i,i+1)=b1;
                    end
                end
    ...
    for i =1:N
        u_dis(1,i+1) = 9/8*u_dis(2,i+1) - 1/8*u_dis(3,i+1) ;
        u_dis(M+2,i+1) = 9/8*u_dis(M+1,i+1) - 1/8*u_dis(M,i+1);
    end
    for j = 1:M+2
        u_dis(j,1) = 9/8*u_dis(j,2) - 1/8*u_dis(j,3) ;
        u_dis(j,N+2) = 9/8*u_dis(j,N+1) - 1/8*u_dis(j,N) ;
    end
	%% Ve nghiem xap xi
