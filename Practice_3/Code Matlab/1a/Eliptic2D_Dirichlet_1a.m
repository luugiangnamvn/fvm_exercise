...
%   bang Phuong phap the tich huu han voi bien Dirichlet thuan nhat
%%                                                                       %%
...
for jj=1:iteration
    ...
    for j=1:M
        for i=1:N
            u_dis(j+1,i+1)=u((j-1)*N+i); %Ma tran se co dang (M+1)x(N+1)
        end
    end
    %% Ve nghiem xap xi
    figure
    ...
    norml2(jj)=sqrt(norml2(jj)); % Sai so trong L^2
    for i=1:N
        for j=1:M
    ...
    normh1(jj)=sqrt(normh1(jj)); % Sai so trong H^1
    ll(jj) = N*M;
    %% Refine mesh
    N=2*N;
    M =2*M;
